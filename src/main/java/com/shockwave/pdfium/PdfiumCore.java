...
public class PdfiumCore {
    ...
    private native long nativeOpenMemDocument(byte[] data, int size, String password);
    ...
    public PdfDocument newDocument(byte [] data) throws IOException {
        return newDocument(data, data.length, null);
    }

    public PdfDocument newDocument(byte [] data, String password) throws IOException {
        return newDocument(data, data.length, password);
    }

    public PdfDocument newDocument(byte [] data, int size, String password) throws IOException {
        PdfDocument document = new PdfDocument();
        synchronized (lock) {
            document.mNativeDocPtr = nativeOpenMemDocument(data, size, password);
        }
