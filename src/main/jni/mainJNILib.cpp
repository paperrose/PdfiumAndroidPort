...
JNI_FUNC(jlong, PdfiumCore, nativeOpenMemDocument)(JNI_ARGS, jbyteArray data, jint size, jstring password){
    if (data == NULL) {
        jniThrowException(env, "java/io/IOException",
                                            "cannot create mem document");
        return -1;
    }
    ...
    FPDF_DOCUMENT document = FPDF_LoadMemDocument(cData, size, cpassword);
